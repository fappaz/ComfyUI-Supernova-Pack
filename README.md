...
| `mode` | `ionian`, `dorian` | Changes the mode, keeping the tonic: `ionian` (major), `dorian`, `phrygian`, `lydian`, `mixolydian`, `aeolian` (natural minor), `locrian`. Each note keeps its scale degree (C minor → C major turns E♭, A♭, B♭ into E, A, B); notes outside the scale keep their pitch. Chords in the key change quality (Fm7 → Fmaj7). Works with `keyscale` and `semitone_offset`. `K:` is written as plain major or minor (C dorian → `K:Cm`), for players that don't know mode names. Use with `explicit_accidentals` so the changed notes are spelled out (`_E`, `=A`). |
| `explicit_accidentals` | on | On (default): every flat, sharp and natural is written on the note itself (E-flat as `_E`) instead of relying on the key signature in `K:`. Same music, but music models that read the note letters, such as YuE, get it right, and mode changes show in the notes. Applies to the whole score, even with nothing else changed: pass your original through the node to get a YuE-friendly version. |
